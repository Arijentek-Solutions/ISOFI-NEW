            ...
            {items.map((item, i) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 rounded-[28px] overflow-hidden shadow-2xl",
        ...
        <div className="mt-8 flex items-center justify-center gap-3">
          ...
          {activeItem.href ? (
            <Link
              ...
              target="_blank"
              rel="noreferrer"
              className="text-black/50 hover:text-black dark:text-white/50 dark:hover:text-white transition ml-2"
...
function DefaultFanCard({ item }: { item: CardStackItem; active: boolean }) {
  return (
    <div className="relative h-full w-full rounded-[24px] overflow-hidden">
      ...
      {/* subtle gradient overlay at bottom for text readability if title or description present */}
