...
const defaultColors = ['#ff2727', '#ff0000', '#ff9090'];
...
export default function LiquidEther({
  ...
  const activeColors = colors || (color0 && color1 && color2 ? [color0, color1, color2] : defaultColors);
  ...
  useEffect(() => {
    ...
    const paletteTex = makePaletteTexture(activeColors);
...
`;
    ...
    if(isBFECC == false){
        ...
        vec2 spot_new4 = spot_new - error / 2.0;
        vec2 vel_2 = texture2D(velocity, spot_new4).xy;
        vec2 spot_old2 = spot_new4 - vel_2 * dt * ratio;
...
`;
    ...
    return () => {
      ...
      paletteTexRef.current = null;
    };
  }, []);

  useEffect(() => {
    // Dynamically update palette texture when colors change
    if (paletteTexRef.current) {
      const arr = activeColors.length === 1 ? [activeColors[0], activeColors[0]] : activeColors;
      const w = arr.length;
      const data = new Uint8Array(w * 4);
      for (let i = 0; i < w; i++) {
        const c = new THREE.Color(arr[i]);
      ...
      paletteTexRef.current.image = { data, width: w, height: 1 } as any;
      paletteTexRef.current.needsUpdate = true;
    }
